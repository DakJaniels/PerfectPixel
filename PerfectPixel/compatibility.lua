...
PP.compatibility = function ()
    local function Compatibility()
        ...
        if LibScrollableMenu then
            local function initializeLibScrollableMenu()
                local lsm = LibScrollableMenu
                local styledControls = {} -- Cache styled controls

                -- Style highlight for menu items
                ...
                -- Hide munge overlays for a control
                local function hideControlMungeOverlays(parentControl)
                    ...
                    {
                        "MungeOverlay",
                        "HeaderBGDividerMungeOverlay",
                        "HeaderDividerSimpleMungeOverlay",
                        "HeaderDividerSimpleDividerMungeOverlay"
                    }

                    for _, name in ipairs(overlayNames) do
                        local control = GetControl(parentControl, name)
                ...
                -- Add PerfectPixel styling to a control
                local function addPPStyle(control, data, templateName)
                    if not control or styledControls[control] then return end
                    ...
                    elseif templateName then
                        ...
                        control:UpdateHighlightTemplate(templateName, "ZO_ThinListHighlight")
                    end

                    styledControls[control] = true
                end

                -- Style dropdown background and scrollbar
                ...
                local dropdownOptions =
                {
                    ...
                    XMLRowTemplates =
                    {
                        [lsm.scrollListRowTypes.LSM_ENTRY_TYPE_HEADER] =
                        {
                            ...
                            rowHeight = 25,
                            setupFunc = function (control, data) addPPStyle(control, data) end
                        },
                ...
                local templateNames =
                {
                    "LibScrollableMenu_ComboBoxHeaderEntry",
                    "LibScrollableMenu_ComboBoxEntry",
                    "LibScrollableMenu_ComboBoxCheckboxEntry",
                    "LibScrollableMenu_ComboBoxRadioButtonEntry"
                }
                for _, templateName in ipairs(templateNames) do
                    addPPStyle(GetControl(templateName), nil, templateName)
                end

                -- Register callbacks
                local function onShowCallback(_, dropdownControl)
                    addPPBackgroundToLSMDropdown(dropdownControl)
                end

                lsm:RegisterCallback("OnMenuShow", onShowCallback, dropdownOptions)
                lsm:RegisterCallback("OnSubMenuShow", onShowCallback, dropdownOptions)

                local contextMenuCallback = lsm.version < "2.40" and "OnContextmenuShow" or "OnContextMenuShow"
                lsm:RegisterCallback(contextMenuCallback, onShowCallback, dropdownOptions)
            end

            initializeLibScrollableMenu()
        end
        -- ===============================================================================================--
        ...
        if IIFA_GUI then
            ...
            PP.Anchor(IIFA_GUI_ListHolder_Slider, --[[#1]] nil, nil, nil, nil, nil, --[[#2]] true, nil, nil, nil, 0, 0)
        ...
        -- ==ZO_ChampionPerks==--
        if ZO_ChampionPerks then
            PP.onDeferredInitCheck(ZO_ChampionPerks, function ()
                PP:CreateBackground(ZO_ChampionPerksActionBarBackground, --[[#1]] nil, nil, nil, 0, 0, --[[#2]] nil, nil, nil, 0, 0)
            end)
        end
        -- ===============================================================================================--
        -- UnregisterForEvent--
